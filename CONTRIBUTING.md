...
- `.github/workflows/ci.yml` is verify-only. It runs `make bootstrap-ci` and `make verify` on pull requests and `main` pushes
- `make bootstrap-ci` reuses the restored `Pods` sandbox when `Pods/Manifest.lock` already matches `Podfile.lock` and falls back to `pod install` only when the cache is stale
...
- Keep the checked-in Xcode project build number at `1` as a harmless baseline. Apple requires `CFBundleVersion` to start above `0`, and App Store Connect only cares that uploaded builds keep increasing for a given release line
