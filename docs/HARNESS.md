...
`boot --run-id <id>` names its device `putio-harness-<platform>-<first 24 characters of id>` and refuses to start when a device with that name already exists, so a colliding ID cannot claim another run's Simulator. Cleanup is registered before creation and targets the exact device the run created; the verified-unowned name is used only if a signal lands before `simctl create` returns. The interruption check uses that exact name to find the owned device and preserves preexisting devices and devices created by other processes.
