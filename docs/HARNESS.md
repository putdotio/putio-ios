...
`journey --platform ios --scenario files-browser` proves the browser with real accessibility input. One XCUITest waits for root folder `0`, taps folder `410`, asserts file `411`, taps the native `Files` Back button, and asserts the restored root. HTTP responses are deterministic fixtures; the app UI, row tap, navigation, and Back control are real.
