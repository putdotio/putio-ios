...
All simulator commands are headless. The harness never opens Simulator.app. `boot`, `launch`, `exercise`, and capture runs create uniquely named devices and pair watchOS with an ephemeral iPhone companion. `--run-id` is included in transient device names and command results so an interrupted attempt remains identifiable. `launch`, `exercise`, and capture wait for a rendered app frame. Before creation, the harness snapshots exact-name matches; post-create failures retain or reconcile newly observed UDIDs for a teardown retry. Every created device is tracked by its exact UDID, then shut down, deleted, and verified absent on success, failure, SIGINT, or SIGTERM without touching preexisting devices. Once one terminal event wins, follow-up termination signals are ignored by the harness and its cleanup subprocesses until teardown finishes. Cleanup diagnostics never replace the primary command or signal-derived exit status. The harness does not offer persistent Simulator sessions; any future persistent mode must return exact UDIDs and an explicit teardown command to the caller. `build` does not create devices.
