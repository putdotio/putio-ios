...
`journey --platform ios --scenario files-browser` proves the runnable alpha loop with real accessibility input. Fifteen unrecorded `1/1` preflights cover:
...
- Audio: tapping a seeded track opens the native Now Playing sheet, plays the local fixture, pauses, changes speed, auto-advances to the folder successor, reaches end of folder, and keeps the chosen speed on reopen. Captures `runtime-audio-player.png`.
