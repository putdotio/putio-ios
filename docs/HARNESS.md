...
- Deep links: cold and warm URL delivery to folder, video, History, and Account; signed-out intent replay, retained-session cold launch, lookup retry, and explicit unavailable routes. Captures `runtime-deep-link-loading.png`, `runtime-deep-link-error.png`, and `runtime-deep-link-folder.png`.
