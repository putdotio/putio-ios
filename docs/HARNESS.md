...
`journey --platform ios --scenario files-browser` proves the runnable alpha loop with real accessibility input. Eighteen unrecorded `1/1` preflights cover:
...
- Playback preferences: with the persisted default (autoplay off) the root video's Up Next suggestion waits past the countdown, VoiceOver labels name the successor on the title, play, and cancel actions, and the manual play-next action opens the successor at its saved position; then proxy-list failure and retry, failed save, refresh-only recovery after a committed save, subtitle-control visibility, the config-backed autoplay toggle whose first save fails and retries, and proxy/subtitle/autoplay persistence across relaunch. Captures `runtime-playback-preferences.png`.
...
- Downloads: "Download" on the seeded root video inventories the multi-audio fixture, the picker shows English and Turkish with the storage estimate, both are selected within budget, the queue shows conversion-free progress through completion, details disclose both stored audio tracks, offline playback selects the preferred language, the position endpoint is seeded to fail so the position stays pending locally, a relaunch keeps the item and the pending position, and returning to the foreground with the endpoint restored syncs it. Removing the item offers local-only removal as the default and a separately worded destructive choice for the put.io original that follows the account's Trash setting; choosing it removes the local copy, reports the seeded first remote failure without claiming success, and retry moves the original to Trash. Captures `runtime-downloads-picker.png`, `runtime-downloads-queue.png`, `runtime-downloads-detail.png`, and `runtime-downloads-remove.png`.
- Chromecast: the Account entry opens the feature-owned Chromecast settings; the first playback-type save fails and retry commits MP4 without flipping the local value early; the receiver row shows the build's receiver app ID. The Files toolbar Cast button connects through the harness stub receiver; tapping a video while connected casts instead of opening the local player, the receiver's first load fails and retry recovers into playing controls, MP4 casting lists put.io subtitles with the default selected and toggles them off, transport pauses and seeks, the throttled position report reaches the seeded endpoint, the bar above the tab bar survives tab changes and reopens the controls, stopping clears the bar, the row's explicit Cast action starts a second cast, disconnecting drops the session, and a disconnected tap opens the local player again. Captures `runtime-cast-settings.png`, `runtime-cast-error.png`, and `runtime-cast-controls.png`.
- Account security: the Security screen enrolls two-factor authentication through the secret, a rejected code that clears the field, the accepted code, and the recovery codes; regenerating recovery codes retries a transient failure; the signed-in apps list keeps this app without a revoke action, retries a failed revoke, and drops the revoked TV; device linking rejects an unknown code and reports the linked app; the accepted code disables two-factor again. Clear Data retries a transient failure and confirms the selected categories, and Destroy Account rejects a wrong password before the accepted one ends the session locally. Captures `runtime-security-recovery-codes.png`, `runtime-security-apps.png`, and `runtime-danger-clear-data.png`.
