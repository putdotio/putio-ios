...
`journey --platform ios --scenario files-browser` proves the runnable alpha loop with real accessibility input. Eight unrecorded `1/1` preflights cover:
...
- Search and restoration: search appends the second result page, opens a folder result, shows empty results and retries a transient failure; a relaunch restores the Files folder, native Back returns to root, and signing out clears the saved folder before the next sign-in. `runtime-search-results.png` is retained.
