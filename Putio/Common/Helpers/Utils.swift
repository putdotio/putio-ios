...
class Utils {
    ...
    static func authorizeNotifications(application: UIApplication) {
        // The permission alert is a system window, so it can land on top of a
        // screenshot capture and no in-app wait can see it coming. Nothing
        // mocked needs push, so nothing mocked should ask.
