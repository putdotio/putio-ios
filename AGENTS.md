...
- Repo scripts are TypeScript and run directly — Node strips the types, so there is no build step and no `dist/`. Stripping is not checking, so `mise run scripts-typecheck` runs `tsc --noEmit` against `scripts/**/*.ts`. It is deliberately not part of `verify-fast`, which stays a sub-second gate that needs no Node; CI runs it in the Linux `verify-tooling` job instead
...
- Use `mise run secrets-setup` only for signed local builds or private support integrations; it writes ignored `Config/Local.xcconfig` from maintainer-supplied `PUTIO_IOS_INFISICAL_*` values
